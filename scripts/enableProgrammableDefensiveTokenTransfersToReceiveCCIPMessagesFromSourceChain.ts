...
async function main() {
  ...
  const sourceChainSelector = (
    config.networks?.baseSepolia as CustomNetworkConfig
  ).ccipChainSelector;

  console.debug(
    `Enabling 'ProgrammableDefensiveTokenTransfersToReceiveCCIPMessages' from source chain ${sourceChainNetwork}, with CCIP Chain Selector ${sourceChainSelector}`
  );
  ...
  // need to call function allowListSourceChain

  const trx = await contract.allowlistSourceChain(
    sourceChainSelector, // Base Sepolia
    true
  );
